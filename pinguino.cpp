...
float rotate_y = 0;
float rotate_x = 0;
float scale = 600.0f; // Ajusta este valor para cambiar el tamaño del triángulo
...
void display() {
    ...
    //IMAGEN ATRAS
    glColor3f(0.2, 0.2, 1.0); //Engloba todo para un mismo color
	...
	//IMAGEN ADELANTE
	glColor3f(0.0, 0.0, 0.0); //Engloba todo para un mismo color
	glBegin(GL_TRIANGLES);
	glVertex3f(199.00/scale, 20.00/scale, -8.00/scale);//1
	glVertex3f(161.00/scale, 20.00/scale, -8.00/scale);
	glVertex3f(180.00/scale, 39.00/scale, -8.00/scale);
